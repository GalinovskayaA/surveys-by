    ...
    <div class="mt-3">
      <div v-if="question.type === 'select'">
        <select
          ...
          class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-base"
        >
      ...
      <div v-else-if="question.type === 'radio'">
        <div v-for="(option, index) in  question.data.options" :key="option.uuid" class="flex items-center">
          <label class="flex items-center mb-2">
            ...
            />
            <span class="ml-3 block text-base font-medium text-gray-700"> {{ option.text }} </span>
          </label>
        </div>
      </div>
